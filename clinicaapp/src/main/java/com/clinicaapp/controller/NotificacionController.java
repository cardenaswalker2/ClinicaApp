...
public class NotificacionController {
    ...
    public ResponseEntity<?> getNotificacionesByEmail(@PathVariable String email) {
        Usuario usuario = usuarioService.findByEmail(email);
        if (usuario == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(notificacionService.getUltimasNotificaciones(usuario.getId(), 20));
    }

    @GetMapping("/usuario/{email}/count")
    public ResponseEntity<Long> getConteoNoLeidas(@PathVariable String email) {
        Usuario usuario = usuarioService.findByEmail(email);
        if (usuario == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(notificacionService.getConteoNoLeidas(usuario.getId()));
    }
