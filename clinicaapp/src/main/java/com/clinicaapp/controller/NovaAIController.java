...
public class NovaAIController {
    ...
    public ResponseEntity<NovaResponse> pensar(@RequestBody NovaRequest request) {
        ...
        if (userId != null) {
            Optional<Usuario> realUserOpt = usuarioService.findById(userId);
            if (realUserOpt.isPresent()) {
                Usuario u = realUserOpt.get();
                // FORZAR EL NOMBRE REAL
                userName = u.getNombre() + " " + (u.getApellido() != null ? u.getApellido() : "").trim();
        ...
        if (groqApiKey != null && !groqApiKey.isEmpty() && !groqApiKey.startsWith("gsk_TU_API_KEY")) {
            try {
                ...
                if (finalMessage != null) {
                    ...
                    guardarLogAsincrono(userId, userName, msg, r.getRespuesta(), "Groq Llama-3.1", r.getIntent());
        ...
        NovaResponse respFallback = generarRespuesta(intent, userName);
        guardarLogAsincrono(userId, userName, msg, respFallback.getRespuesta(), "Regex Heuristics", intent);
        return ResponseEntity.ok(respFallback);
    }
    ...
    private void guardarLogAsincrono(String userId, String userName, String prompt, String respuesta, String motor, String intent) {
        new Thread(() -> {
            try {
                LogInferencia log = new LogInferencia(
                    userId == null ? "ANON" : userId,
                    userName,
                    prompt == null || prompt.isEmpty() ? "<VACÍO>" : prompt,
                    respuesta,
                    motor,
                    intent == null ? "UNKNOWN" : intent
                );
                logInferenciaRepo.save(log);
            } catch (Exception e) {
                System.err.println("[NovaAI] Error guardando log de inferencia: " + e.getMessage());
            }
        }).start();
    }
}