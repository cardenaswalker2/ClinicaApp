...
public class CheckoutController {
    ...
    public String checkout(@PathVariable String citaId, Model model) {
        ...
        // Si el costo de la cita es menor al mínimo de Stripe ($0.50 USD / 50 centavos),
        // se procesa automáticamente como cortesía/gratuito sin llamar a Stripe.
        if (cita.getCosto() == null || cita.getCosto() < 0.50) {
            log.info("Costo de la cita (${}) es menor al límite mínimo de Stripe ($0.50 USD). Procesando como pago gratuito de cortesía.", cita.getCosto());
            String dummyPaymentId = "FREE-" + citaId + "-" + System.currentTimeMillis();
            cita.setPaymentIntentId(dummyPaymentId);
            citaRepository.save(cita);
            return "redirect:/pago-exitoso?payment_intent=" + dummyPaymentId;
        }
        ...
        try {
            ...
            // --- Nuevos datos para la versión Premium ---
            Usuario usuario = usuarioService.findById(cita.getUsuarioId()).orElse(new Usuario());
            com.clinicaapp.model.Clinica clinica = clinicaService.findById(cita.getClinicaId()).orElse(new com.clinicaapp.model.Clinica());
            
            DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("d 'de' MMMM, yyyy", new Locale("es", "ES"));
            DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("h:mm a");
            
            String nombreDoctor = "Especialista Asignado";
            if (clinica.getEquipoMedico() != null && !clinica.getEquipoMedico().isEmpty()) {
                nombreDoctor = clinica.getEquipoMedico().get(0).get("nombre");
            }
            ...
            // Atributos dinámicos para el nuevo diseño
            model.addAttribute("fechaCita", cita.getFechaHora().format(dateFormatter));
            model.addAttribute("horaCita", cita.getFechaHora().format(timeFormatter));
            model.addAttribute("nombreDoctor", nombreDoctor);
            model.addAttribute("consultorio", "Sala de Atención - Piso 1");
