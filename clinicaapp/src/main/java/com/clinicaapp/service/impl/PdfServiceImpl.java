...
public ByteArrayInputStream generatePdfFromTemplate(String templateName, Map<String, Object> data) {
    ...
    try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
        ...
        // --- SOLUCIÓN AL ERROR java.io.EOFException (mstmc.ttf) ---
        // En lugar de escanear C:\Windows\Fonts, usamos una fuente específica cargada desde recursos.
        try {
            byte[] fontBytes = getClass().getResourceAsStream("/fonts/Roboto.ttf").readAllBytes();
            builder.useFont(() -> new ByteArrayInputStream(fontBytes), "Roboto");
        } catch (Exception fontEx) {
            log.warn("No se pudo cargar la fuente Roboto, se usarán las fuentes base: {}", fontEx.getMessage());
        }

        builder.useFastMode();
        // builder.testMode(true); // Ya no es necesario si no escaneamos el sistema
